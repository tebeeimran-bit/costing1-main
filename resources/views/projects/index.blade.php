            ...
            <tbody>
                @forelse($pagedGroups as $group)
                    ...
                    <tr class="group-row" id="{{ $rowId }}Main">
                        ...
                        <td><div class="project-main"><strong>{{ $group->project_name }}</strong><small>{{ $group->business_category }}</small>@foreach($group->shared_a00_labels as $sharedA00)<div class="shared-a00-note" title="{{ $sharedA00->number }} — digunakan bersama oleh {{ $sharedA00->project_count }} project"><span class="shared-a00-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></span><span class="shared-a00-copy"><span>A00 Gabung</span><strong title="{{ $sharedA00->number }}">{{ $sharedA00->number }}</strong></span><span class="shared-a00-count"><b>{{ $sharedA00->project_count }}</b><small>Project</small></span></div>@endforeach</div></td>
