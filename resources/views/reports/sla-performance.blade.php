...
<div class="sla-page">
    ...
    <section class="sla-panel sla-history">
        <div class="sla-section-head sla-history-head"><div><h3>Historical SLA Trend</h3><p>Snapshot kepatuhan {{ $historyRange }} hari terakhir, lengkap dengan evaluasi tahap dan PIC.</p></div><form method="GET"><input type="hidden" name="stage" value="{{ $stageFilter }}"><input type="hidden" name="status" value="{{ $statusFilter }}"><select name="history_range" onchange="this.form.submit()"><option value="30" @selected($historyRange===30)>30 hari</option><option value="90" @selected($historyRange===90)>90 hari</option><option value="365" @selected($historyRange===365)>12 bulan</option></select></form></div>
        <div class="sla-history-chart">
            @forelse($history as $point)@php($pointDate=\Carbon\Carbon::parse($monthly ? $point->date.'-01' : $point->date))<div title="{{ $pointDate->format($monthly?'M Y':'d M Y') }} · {{ $point->overdue }} overdue"><span>{{ $point->compliance }}%</span><i style="height:{{ max(5,$point->compliance) }}%"></i><small>{{ $pointDate->format($monthly?'M y':'d/m') }}</small></div>@empty<div class="sla-empty">Snapshot pertama akan dibuat oleh scheduler harian.</div>@endforelse
        </div>
        <div class="sla-history-breakdown"><div><h4>Historis per Tahap</h4>@forelse($historicStageSummary as $item)<article><b>{{ ucfirst($item->label) }}</b><span>{{ $item->compliance }}% · {{ $item->overdue }} overdue · aging {{ $item->average_aging }} hari</span></article>@empty<p>Belum ada data historis.</p>@endforelse</div><div><h4>Historis per PIC</h4>@forelse($historicPicSummary as $item)<article><b>{{ $item->label }}</b><span>{{ $item->compliance }}% · {{ $item->overdue }} overdue · {{ $item->samples }} snapshot</span></article>@empty<p>Belum ada data historis.</p>@endforelse</div></div>
    </section>
...
.sla-history-head{display:flex;align-items:center;justify-content:space-between;gap:12px}.sla-history-head select{padding:8px 10px;border:1px solid #cddce8;border-radius:8px;background:#fff;color:#31516f;font-size:10px}.sla-history-breakdown{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:17px}.sla-history-breakdown h4{margin:0 0 7px;font-size:11px}.sla-history-breakdown article{display:flex;justify-content:space-between;gap:10px;padding:7px 0;border-top:1px solid #e8eef3;font-size:9px}.sla-history-breakdown span{color:#6d8195;text-align:right}@media(max-width:650px){.sla-history-head{align-items:flex-start;flex-direction:column}.sla-history-breakdown{grid-template-columns:1fr}}
</style>
@endsection
