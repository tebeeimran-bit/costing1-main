...
.dc-field select{width:100%;height:38px;box-sizing:border-box;transition:border-color .15s,box-shadow .15s}.dc-field select:focus{border-color:#2563eb;box-shadow:0 0 0 3px rgba(37,99,235,.1);outline:none}
   ...
   <section class="dc-section">
    ...
    <div class="dc-grid">
     ...
     <div class="dc-field {{ in_array($field, ['customer','part_name','drawing_remark','return_remark','crusher_remark']) ? 'dc-field-wide' : '' }}">
      ...
      @if(isset($formOptions[$field]))
       <select class="dc-input" id="f_{{ $field }}" name="{{ $field }}">
        <option value="">Pilih {{ $columns[$field] }}</option>
        @foreach($formOptions[$field] as $option)<option value="{{ $option }}">{{ $option }}</option>@endforeach
       </select>
      @else
